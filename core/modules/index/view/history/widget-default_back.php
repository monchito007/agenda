...
if($_GET["q"]!=""||$_GET["project_id"]!="" ||$_GET["category_id"]!="" ){
	...
	$sql .= " date_at = \"".$_GET["date_at"]."\"";
        ...
        $sql .= " and (( date_at < CURRENT_DATE() and time_at < CURRENT_TIME() ) OR (date_at < CURRENT_DATE())) order by `date_at`,`time_at` DESC";

		$users = EventData::getBySQL($sql);
                
}else{
    
        $sql = "select * from event where (( date_at < CURRENT_DATE() and time_at < CURRENT_TIME() ) OR (date_at < CURRENT_DATE())) order by `date_at`,`time_at` DESC";
...
'GROUP by MONTH(a.date_at),YEAR(a.date_at)';

$sql_visitas = 'SELECT b.name as "Categoria",COUNT(*) as "cantidad", MONTHNAME(a.date_at) as "nombre_mes", MONTH(a.date_at) as "Mes", YEAR(a.date_at) as "Año" from event as a,category as b '.
'WHERE (( a.date_at < CURRENT_DATE() and a.time_at < CURRENT_TIME() ) '.
'OR (a.date_at < CURRENT_DATE())) '.
'AND a.category_id=2 '.
'AND a.category_id=b.id '. 
...
$visitas = EventData::getBySQL($sql_visitas);
...
<script type="text/javascript">
    ...
    var json_visitas = <?php echo json_encode($visitas);?>;
    
    google.charts.load('current', {'packages':['bar']});
    google.charts.setOnLoadCallback(drawChart);
    
    datachart=[['Mes', 'Firmas', 'Visitas']];
    datachart=[['Mes', 'Firmas']];
    
    for(var x=0;x<json_firmas.length;x++){
        
        datachart.push([json_firmas[x].nombre_mes,parseInt(json_firmas[x].cantidad)]);
